$(function () {
	...
	var ulwidth = lilen * 54 + (lilen - 1) * 8;
...
(function(win){
    ...
    win.slider={
        ...
        barAnimate:function(callback){
            var that=this;
            that.bar.animate({'width': '100%'}, 5500, 'linear',callback);
        },
        ...
        changeBg:function(color, callback){
            var that=this;
            that.bannerBg.fadeOut(250, function () {
                $(this).css('background-image', 'url('+color+')').delay(50).fadeIn(250, callback);
            });
        },
